...
module.exports = {
  ...
  getExperimentAnswer: () =>
    conn.query('SELECT deviceId, schedule, question, `index`, text FROM experiment_answer' +
        ' WHERE final = 1 AND createdAt > "2018-01-01 00:00:00" ORDER BY createdAt'),
  ...
  getRounds: () =>
    conn.query('SELECT deviceId, round, blackDuration, redDuration, recordedDuration, timeBetweenMountAndStart, schedule' +
        ' FROM round WHERE createdAt > "2018-01-01 00:00:00" ORDER BY createdAt'),
  getRoundsForAllColumns: () =>
    conn.query('SELECT * FROM round WHERE createdAt > "2018-01-01 00:00:00" ORDER BY deviceId, createdAt'),
};
